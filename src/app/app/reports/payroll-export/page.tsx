...
export default function PayrollExportPage() {
  ...
  const handleExportPDF = useCallback(async () => {
    ...
    // ── Branded header ──────────────────────────────────────────────────
    // Left: ShiftOps wordmark + tenant name
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(17, 24, 39);
    doc.text("ShiftOps", 40, 40);
    if (tenant?.name) {
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(107, 114, 128);
      doc.text(tenant.name, 40, 54);
    }
    ...
    doc.text("Sage Pastel Payroll Export", pageWidth - 40, 38, { align: "right" });
    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(107, 114, 128);
    if (period) doc.text(`Period: ${period}`, pageWidth - 40, 52, { align: "right" });
    doc.text(`Generated: ${new Date().toLocaleString("en-ZA")}`, pageWidth - 40, 64, { align: "right" });

...
export default function DailyBankingPage() {
  ...
  const handleRun = useCallback(
    async (f: ReportFilters) => {
      ...
      const { data: cashups } = await supabase
        .from("daily_cashups")
        .select("*, cashup_online_payments(amount), cashup_expenses(amount)")
      ...
      if (cashups) {
        ...
        for (const c of cashups as (DailyCashup & {
          cashup_online_payments: { amount: number | null }[];
          cashup_expenses: { amount: number | null }[];
        })[]) {
          const gross = c.gross_turnover ?? 0;
          const disc = c.discounts ?? 0;
          const deliv = c.delivery_charges ?? 0;
          const cc = c.credit_cards ?? 0;
          const debtors = c.debtors ?? 0;
          const onlinePayments = (c.cashup_online_payments ?? []).reduce(
            (s, op) => s + (op.amount ?? 0),
            0
          );
          const expenses = (c.cashup_expenses ?? []).reduce(
            (s, ex) => s + (ex.amount ?? 0),
            0
          );
          const banked = c.cash_banked ?? 0;
          const batch = c.cc_batch_total ?? 0;
          // Daily Banking = Turnover - Discounts + Delivery Charges - Credit Cards - Debtors - Online Payments - Expenses
          const expectedBanking = gross - disc + deliv - cc - debtors - onlinePayments - expenses;
          const variance = expectedBanking - banked;

          const existing = byDate.get(c.date);
          if (existing) {
            existing.gross_turnover += gross;
            existing.discounts += disc;
            existing.delivery_charges += deliv;
            existing.credit_cards += cc;
            existing.debtors += debtors;
            existing.online_payments += onlinePayments;
            existing.expenses += expenses;
            existing.cash_banked += banked;
            existing.cc_batch_total += batch;
            existing.expected_banking += expectedBanking;
  ...
  const handleExportCSV = useCallback(() => {
    const headers = ["Date", "Gross Turnover", "Discounts", "Delivery Charges", "Credit Cards", "Debtors", "Online Payments", "Expenses", "Expected Banking", "Cash Banked", "CC Batch", "Variance"];
    const rows = data.map((r) => [r.date, r.gross_turnover, r.discounts, r.delivery_charges, r.credit_cards, r.debtors, r.online_payments, r.expenses, r.expected_banking, r.cash_banked, r.cc_batch_total, r.variance]);
  ...
  const totals = data.reduce(
    (acc, r) => ({
      ...
      delivery_charges: acc.delivery_charges + r.delivery_charges,
      credit_cards: acc.credit_cards + r.credit_cards,
      debtors: acc.debtors + r.debtors,
      online_payments: acc.online_payments + r.online_payments,
      expenses: acc.expenses + r.expenses,
      expected_banking: acc.expected_banking + r.expected_banking,
      cash_banked: acc.cash_banked + r.cash_banked,
      cc_batch_total: acc.cc_batch_total + r.cc_batch_total,
      variance: acc.variance + r.variance,
    }),
    { gross_turnover: 0, discounts: 0, delivery_charges: 0, credit_cards: 0, debtors: 0, online_payments: 0, expenses: 0, expected_banking: 0, cash_banked: 0, cc_batch_total: 0, variance: 0 }
  );
  ...
  return (
    <ReportWrapper title="Daily Banking Summary" onRun={handleRun} onExportCSV={handleExportCSV}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Expected Banking" value={formatCurrency(totals.expected_banking)} icon={<Banknote className="h-5 w-5" />} />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-surface-2">
                {["Date", "Gross Turnover", "Discounts", "Delivery", "Credit Cards", "Debtors", "Online Pmts", "Expenses", "Expected Banking", "Cash Banked", "Variance"].map((h) => (
            ...
            <tbody>
              {data.map((row) => (
                <tr key={row.date} className="border-b border-base-200 hover:bg-surface-2 transition-colors">
                  ...
                  <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(row.expenses)}</td>
                  <td className="px-4 py-2 text-right font-mono text-base-900 font-semibold">{formatCurrency(row.expected_banking)}</td>
            ...
            <tfoot>
              <tr className="bg-surface-2 font-semibold">
                ...
                <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(totals.delivery_charges)}</td>
                <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(totals.credit_cards)}</td>
                <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(totals.debtors)}</td>
                <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(totals.online_payments)}</td>
                <td className="px-4 py-2 text-right font-mono text-base-900">{formatCurrency(totals.expenses)}</td>
                <td className="px-4 py-2 text-right font-mono text-base-900 font-semibold">{formatCurrency(totals.expected_banking)}</td>
